...
interface SectionHeaderProps {
    ...
    count: number;
    color: string;
    onAddTask: (newTask: any) => void; // Assuming you want to pass a new task object
    hoverColor?: string; // Optional prop for hover color
    activeColor?: string; // Optional prop for active color
}
...
const SectionHeader: React.FC<SectionHeaderProps> = ({
    ...
    color,
    onAddTask,
    hoverColor = "hover:bg-[#FFFAE9]", // Default hover color if not provided
    activeColor = "active:bg-[#FFF0BC]", // Default active color if not provided
}) => {
    ...
    const addTask = () => {
        ...
        closeModal();
        setNewTask({ title: '', phase: '', dateRange: '', daysLeft: '', color: '' });
    };
            ...
            <div className="flex bg-white rounded-xl h-[40px] mt-[-37px] z-10 items-center px-5 gap-5">
                <div className="flex flex-row w-full justify-between">
                    <div className="flex items-center gap-3">
                        <p style={{ color: "#000000", fontWeight: 600, fontSize: 17 }}>{title}</p>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        >
                            <FiPlus 
                                onClick={openModal} // Change to openModal to show the modal
                                style={{ color }} 
